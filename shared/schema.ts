...
export const datasets: Dataset[] = [
  {
    id: "cinematic-camera-motion-kit",
    name: "Cinematic Camera Motion Kit",
    nameKo: "시네마틱 카메라 모션 키트",
    description: "Camera path/control traces with timing, easing, and FOV changes for 3D animation pipelines; ideal for auto-generated scene choreography.",
    descriptionKo: "3D 애니메이션 파이프라인을 위한 카메라 경로·타이밍·이징·FOV 변화 트레이스로, 자동 생성 씬 카메라 워크 학습에 적합합니다.",
    category: "Analytics",
    price: 1610,
    records: "1.8M",
    accuracy: "96.4%",
    ...
    features: [
      "Bezier/spline camera paths with easing tags",
      "Multi-camera rig (dolly, crane, orbit) states",
      "Depth-of-field + exposure envelopes",
      "Scene graph references for synchronization",
    ],
    sampleFields: [
      { name: "take_id", type: "string", description: "Camera take identifier" },
      { name: "timestamp", type: "float", description: "Seconds from start" },
      { name: "transform", type: "array", description: "Position/rotation per frame" },
      { name: "easing", type: "string", description: "easeInOutQuad 등 이징 태그" },
      { name: "fov", type: "float", description: "Field of view degrees" },
    ],
    sampleData: [
      { take_id: "CAM-001", timestamp: 0.32, transform: "...", easing: "easeInOutQuad", fov: 42.0 },
      { take_id: "CAM-002", timestamp: 1.05, transform: "...", easing: "cubicIn", fov: 36.5 },
    ],
  },
  {
    id: "hires-3d-modeling-dataset",
    name: "High-Resolution 3D Modeling Learning Dataset for 3D Geometry and Scene Understanding",
    nameKo: "3D 기하·장면 이해를 위한 고해상도 3D 모델링 학습 데이터셋",
    description: "High-resolution 3D modeling training data for understanding 3D geometry and scenes; ideal for computer vision and deep learning models.",
    descriptionKo: "3D 기하와 장면 이해를 위한 고해상도 3D 모델링 학습 데이터로, 컴퓨터 비전 및 딥러닝 모델 학습에 적합합니다.",
    category: "Analytics",
    price: 850,
    records: "2.3M",
    accuracy: "97.5%",
    format: "PLY, OBJ, Parquet",
    updateFrequency: "Quarterly",
    features: [
      "High-resolution 3D mesh data",
      "Scene understanding annotations",
      "Geometry segmentation labels",
      "Multi-view consistency data",
    ],
    sampleFields: [
      { name: "model_id", type: "string", description: "3D model identifier" },
      { name: "vertices", type: "integer", description: "Number of vertices" },
      { name: "faces", type: "integer", description: "Number of faces" },
      { name: "scene_label", type: "string", description: "Scene category" },
      { name: "geometry_type", type: "string", description: "Geometry classification" },
    ],
    sampleData: [
      { model_id: "3DM-001", vertices: 45000, faces: 89000, scene_label: "indoor", geometry_type: "furniture" },
      { model_id: "3DM-002", vertices: 32000, faces: 63000, scene_label: "outdoor", geometry_type: "building" },
    ],
  },
  {
    ...
    description: "Sentiment-labeled financial news covering markets, earnings, and macro events; ideal for trading and risk models.",
    descriptionKo: "시장·실적·거시 이벤트를 아우르는 감성 라벨 금융 뉴스 데이터셋으로 트레이딩·리스크 모델에 적합합니다.",
    category: "Finance",
    price: 3190000,
    records: "2.5M",
    accuracy: "97.8%",
    ...
    features: [
      "Headline & body sentiment labels",
      "Entity tags for issuers/sectors",
      "Event classification (earnings, guidance, macro)",
      "Timestamped for intraday modeling",
    ],
    sampleFields: [
      { name: "article_id", type: "string", description: "Unique article id" },
      { name: "headline", type: "string", description: "뉴스 제목" },
      { name: "body", type: "string", description: "본문 요약/전문" },
      { name: "sentiment", type: "string", description: "pos/neg/neutral" },
      { name: "event_type", type: "string", description: "earnings/guidance/macro 등" },
    ],
    sampleData: [
      { article_id: "FN-001", headline: "Tech Firm Beats Earnings", body: "...", sentiment: "pos", event_type: "earnings" },
      { article_id: "FN-002", headline: "Central Bank Signals Pause", body: "...", sentiment: "neutral", event_type: "macro" },
    ],
  },
  {
  ...
  {
    id: "procedural-geometry-recognition",
    name: "Procedural Geometry Recognition Pack",
    nameKo: "절차적 기하 인식/생성 팩",
    description: "Labeled 3D primitives, closed surfaces, and boolean ops with JSON schemas for shape recognition and procedural generation.",
    descriptionKo: "3D 기본 도형·폐곡면·불리언 연산이 라벨된 데이터와 JSON 스키마를 제공해 형태 인식과 절차적 생성 모델 학습에 적합합니다.",
    category: "Analytics",
    price: 3190000,
    records: "520K",
    accuracy: "95.1%",
    ...
    features: [
      "Primitives (cube, sphere, torus) with params",
      "Closed-surface annotations and watertight flags",
      "Boolean CSG operation traces",
      "Schema examples for geometry JSON IO",
    ],
    sampleFields: [
      { name: "shape_id", type: "string", description: "Shape identifier" },
      { name: "primitive", type: "string", description: "cube/sphere/torus 등" },
      { name: "params", type: "object", description: "size/radius/segments 등 파라미터" },
      { name: "is_watertight", type: "boolean", description: "폐곡면 여부" },
      { name: "boolean_op", type: "string", description: "union/diff/intersection" },
    ],
    sampleData: [
      { shape_id: "GEOM-001", primitive: "torus", params: { r1: 1.0, r2: 0.3 }, is_watertight: true, boolean_op: "union" },
      { shape_id: "GEOM-002", primitive: "cube", params: { size: 2.0 }, is_watertight: true, boolean_op: "difference" },
    ],
  },
  {
    id: "geospatial-poi",
  ...
  {
    id: "spatial-perception-weights",
    name: "Spatial Perception Weight Curations",
    nameKo: "공간 지각 가중치 컬렉션",
    description: "Curated transformer/diffusion weights focused on spatial reasoning, occlusion handling, and 3D layout consistency for animation pipelines.",
    descriptionKo: "공간 추론·가림 처리·3D 레이아웃 일관성에 초점을 둔 트랜스포머/확산 가중치 큐레이션으로 애니메이션 자동화 파이프라인에 적합합니다.",
    category: "Analytics",
    price: 1000000,
    records: "240",
    accuracy: "97.8%",
    ...
    updateFrequency: "Monthly",
    features: [
      "Checkpoints tuned for spatial coherence",
      "LoRA/adapters for occlusion-aware rendering",
      "Eval clips with depth/normal consistency",
      "Prompt templates for camera + layout prompts",
    ],
    sampleFields: [
      { name: "checkpoint_id", type: "string", description: "Weight identifier" },
      { name: "focus", type: "string", description: "occlusion/layout/depth" },
      { name: "base_model", type: "string", description: "Backbone family" },
      { name: "format", type: "string", description: "safetensors/pt" },
      { name: "eval_score", type: "float", description: "Spatial consistency metric" },
    ],
    sampleData: [
      { checkpoint_id: "SP-01", focus: "layout", base_model: "transformer-xl", format: "safetensors", eval_score: 0.94 },
      { checkpoint_id: "SP-02", focus: "occlusion", base_model: "diffusion-large", format: "pt", eval_score: 0.92 },
    ]
  },
  {
    id: "b2b-credit-risk",
